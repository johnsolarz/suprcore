 ...
 * @todo https://github.com/retlehs/core/issues/461
 *
 * Rewrite:
 ...
 *
 * @link https://github.com/retlehs/core/wiki/Nginx
 * @link https://github.com/retlehs/core/wiki/Lighttpd
 */
...
if (stristr($_SERVER['SERVER_SOFTWARE'], 'apache') || stristr($_SERVER['SERVER_SOFTWARE'], 'litespeed') !== false) {
  ...
  function core_htaccess_writable() {
    if (!is_writable(get_home_path() . '.htaccess')) {
      if (current_user_can('administrator')) {
        add_action('admin_notices', create_function('', "echo '<div class=\"error\"><p>" . sprintf(__('Please make sure your <a href="%s">.htaccess</a> file is writable ', 'core'), admin_url('options-permalink.php')) . "</p></div>';"));
      }
    }
  }

  add_action('admin_init', 'core_htaccess_writable');

  function core_add_rewrites($content) {
    global $wp_rewrite;
    $core_new_non_wp_rules = array(
      'css/(.*)'      => THEME_PATH . '/css/$1',
      'js/(.*)'       => THEME_PATH . '/js/$1',
      'img/(.*)'      => THEME_PATH . '/img/$1',
      'plugins/(.*)'  => RELATIVE_PLUGIN_PATH . '/$1'
    );
    $wp_rewrite->non_wp_rules = array_merge($wp_rewrite->non_wp_rules, $core_new_non_wp_rules);
    return $content;
  }

  function core_clean_urls($content) {
  ...
  if (!is_multisite() && !is_child_theme() && get_option('permalink_structure')) {
    if (current_theme_supports('rewrite-urls')) {
      add_action('generate_rewrite_rules', 'core_add_rewrites');
    }

    if (current_theme_supports('h5bp-htaccess')) {
      add_action('generate_rewrite_rules', 'core_add_h5bp_htaccess');
    }
    ...
    if (!is_admin() && current_theme_supports('rewrite-urls')) {
      ...
      add_filters($tags, 'core_clean_urls');
    }
  }

  // Add the contents of h5bp-htaccess into the .htaccess file
  function core_add_h5bp_htaccess($content) {
    global $wp_rewrite;
